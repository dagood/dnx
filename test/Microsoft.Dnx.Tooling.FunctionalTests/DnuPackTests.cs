...
namespace Microsoft.Dnx.Tooling
{
    public class DnuPackTests : DnxSdkFunctionalTestBase
    {
        ...
        public void DnuPack_P2PDifferentFrameworks(DnxSdk sdk)
        {
            var projectStructure = new Dir
            {
                ["P1"] = new Dir
                {
                    ...
                    ["BaseClass.cs"] = "public class BaseClass { public virtual void Test() { } }"
                }
                ["P2"] = new Dir
                {
                    ["project.json"] = new JObject()
                    ...
                    ["TestClass.cs"] = "public class TestClass : BaseClass { public override void Test() { } }"
                }
            };

            var basePath = GetTestDir(sdk);
            projectStructure.Save(basePath);

            sdk.Dnu.Restore(basePath).EnsureSuccess();
            sdk.Dnu.Pack(
                projectDir: Path.Combine(basePath, "P2"),
                outputPath: Path.Combine(basePath, "Output")).EnsureSuccess();
        }

        [Theory]
        [MemberData(nameof(DnxSdks))]
        public void DnuPack_NoArgs(DnxSdk sdk)
        {
            var projectStructure = new Dir
            {
            ...
            string expectedDNX =
            $@"Building ProjectName for DNX,Version=v4.5.1
              Using Project dependency ProjectName 1.0.0
                Source: {projectPath}/project.json".Replace('/', Path.DirectorySeparatorChar);
            string expectedDNXCore =
            $@"Building ProjectName for DNXCore,Version=v5.0
              Using Project dependency {0} 1.0.0
                Source: {projectPath}/project.json".Replace('/', Path.DirectorySeparatorChar);
            string expectedNupkg =
            $@"ProjectName -> {projectPath}/bin/Debug/ProjectName.1.0.0.nupkg
            ProjectName -> {projectPath}/bin/Debug/ProjectName.1.0.0.symbols.nupkg".Replace('/', Path.DirectorySeparatorChar);

            sdk.Dnu.Restore(basePath).EnsureSuccess();
            var result = sdk.Dnu.Pack(Path.Combine(basePath, "ProjectName")).EnsureSuccess();

            Assert.Empty(result.StandardError);
            Assert.Contains(expectedDNX, result.StandardOutput);
            Assert.Contains(expectedDNXCore, result.StandardOutput);
            Assert.Contains(expectedNupkg, result.StandardOutput);
            Assert.True(Directory.Exists($"{projectPath}/bin"));
        }
